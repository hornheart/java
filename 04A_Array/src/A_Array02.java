...
public class A_Array02 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 실제 리터럴값을 곧 바로 담을 수 있는 변수를 일반변수라고 이야기 한다.
		 * 주소값을 담고 있는 변수는 참조 변수 (레퍼런스 변수)라고 표현
		 * 
		 * 기본자료형(boolean, int, char, byte, long, double 둥둥)으로 선언된 변수
		...
		// => 따라서 공간이 만들어질 때 JVM이 기본값으로라도 초기과를 진행해준다.
		
		System.out.println(iArr); // 배열의 자료형 + @ +  주소값의 16진수형태
		System.out.println(iArr.hashCode()); // 주소값의 10진수 형채
		System.out.println("안녕하세요".length());
		System.out.println(iArr.length);  // 배열의 길이
		
		int[] arr = null; //아무것도 참조하고 있지 않음
		// 주소값을 담은 참조변수의 기본값이 null이다.
		System.out.println(arr);
		
		System.out.println(arr.length);
		/*
		 * null을 가지고 참조변수에 접근하여 다른 정보를 구하고자 한다면
		 * 항상 오류가 발행한다 => NullpointException
		 ...
		 * : 배열에 부적절한 인덱스 제기기 발생 (범위에서 벗어난 인덱스 제시)
		 * System.out.println(arr[5]);
		 */
		...
		/*
		 * 
		 * 연결이 끊어진 기존 배열은 Heap영역에 둥둥 떠다님 (어디에도 참조되어 있지않아 사용되지 않음 ==> 필요없는 존재)
		 * => 일정시간이 지나면 "가비지컬레터(GC)"가 알아서 지워줌
		 * => 자바에서의 "자동메모리 관리" 특징 == 개발자가 코드작성에 만 집중할 수 있다.
		 * 
		 */
		...
		// 배열 선언, 할당, 초기와 전부 동시에 진행하기
		int[] arr2 = new int[] {1,2,3,4}; // 표현법1
		int[] arr3 = {1,2,3,4}; // 표현법2
		
		System.out.println(arr2 == arr3); // false -> 각각의 변수에 담겨있는 주소값을 비교기 때문
